...
{
   plistExitStatus(thread_current()->tid);
}
...
int process_execute(const char *command_line)
{
   ...
   arguments.parentID = thread_current()->tid;
   sema_init(&arguments.execSema, 0);
   ...
   sema_down(&arguments.execSema);

   process_id = thread_id;
   if (process_id != -1) // if thread created successfully insert our created thread into the global process list
                         // p_map_insert(process_id, );

      // map_insert(thread_id......)
...
start_process(struct parameters_to_start_process *parameters)
{
   ...
   if (success)
   {
      ...
      parameters->processId = thread_current()->tid;
      parameters->succ = true;
   }
   ...
   */
   if (!success)
   {
      parameters->succ = false;
      parameters->processId = -1;
      sema_up(&parameters->execSema);
      thread_exit();
   };
   // threadInfo* newProcesss (10,false,0,semaphore);
   struct procInfo *proc = malloc(sizeof *proc); // starfoullah.......

   proc->parent = parameters->parentID;

   proc->free = false;
   proc->exitStatus = -1;
   proc->inUse = false;
   proc->waited = false;
   sema_init(&proc->exitSema, 0);

   thread_current()->tid = p_map_insert(proc);
...
int process_wait(int child_id)
{
   ...
   struct procInfo *child = p_map_find(child_id);

   if (&child != NULL && thread_current()->tid == &child->parent)
   {
      sema_down(&child->exitSema);
      status = &child->exitStatus;
      p_map_remove(child_id);
      free(child);
   }
...
bool do_free(key_t k UNUSED, value_t v, int aux UNUSED)
{
   ...
   return true;   /*  and remove from list in map */
}
...
void process_cleanup(void)
{
   ...
   map_remove_if(&thread_current()->opened_files, do_free, 0);
   struct procInfo *child = p_map_find(cur->tid);
   sema_up(&child->exitSema);
}
